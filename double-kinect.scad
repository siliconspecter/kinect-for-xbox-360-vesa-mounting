...
total_length_mm = kinect_length_mm * 2 + kinect_cable_spacing_mm;

difference() {
    union() {
        translate([0, total_length_mm / 2, tube_retainer_length_mm]) {
            rotate([90, 0, 0]) {
                linear_extrude(total_length_mm) {
                    polygon([
    ...
    translate([kinect_screw_x_spacing_mm / 2, kinect_screw_y_spacing_mm / 2 + kinect_cable_spacing_mm / 2 + kinect_length_mm / 2, 0]) {
        screw_hole();
    };
